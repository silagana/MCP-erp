...
| 9 | Configurar WhatsApp Cloud API en Meta: app creada, token permanente (system user), webhook verificado y con el campo `messages` suscripto (había que suscribir explícitamente la app al WABA vía `POST /{waba_id}/subscribed_apps` — no quedaba hecho solo con la pantalla de configuración). Validado end-to-end: mensaje real de WhatsApp → webhook → orchestrator → Claude → tools → respuesta enviada. Único bache: el número de teléfono de prueba (sandbox) tiene una lista de destinatarios autorizados con un bug conocido de Meta para números argentinos (guarda el formato viejo con "15" en vez del `549` moderno) — no aplica una vez que haya un número real de producción, sin lista de autorizados. **Falta:** conseguir/verificar un número de WhatsApp Business real para producción (no el de prueba) y, más adelante, el primer WhatsApp Flow (alta/cobro) en vez de todo a texto libre | ✅ validado (sandbox) |
...
| 14 | Canal de prueba por Telegram (`app/telegram_webhook.py`) — router montado en el **mismo servicio/proceso** que WhatsApp (no un servicio Railway separado — se probó eso y trajo problemas: confundir consolas de dos servicios, y una vez el Custom Start Command del servicio principal quedó apuntando a Telegram, cortando WhatsApp sin querer). "Desactivar" Telegram es un solo `curl .../deleteWebhook`, no hace falta tocar Railway. Reusa el mismo orchestrator/tools/permisos. Vinculación por `/vincular <telefono>` a un usuario ya existente | ✅ hecho |
| 15 | Dashboard de KPIs (`app/dashboard.py`): alumnos, ingresos, morosidad, con gráficos (Chart.js) e identidad visual real de St. Clare's (`docs/BRAND.md`). Tool `generar_dashboard` (respeta rol: owner/administrativo ven todo, alumno ve solo su cuenta) + tabla `reporte_token` (link con vencimiento de 24hs que graba el alcance/permiso del que lo pidió). Cron semanal (`scripts/enviar_resumen_semanal.py`) — **solo por Telegram** por ahora, mismo motivo que la etapa 12 (comunicación saliente de WhatsApp en pausa). Probado end-to-end localmente con datos de ejemplo (queries, render HTML, ruta FastAPI, envío del cron) — falta probarlo contra Railway real | ✅ hecho (falta desplegar) |
...
**Próximo paso inmediato:** desplegar la migración del dashboard contra Railway y probar `generar_dashboard` end-to-end (bloqueado hasta que Anthropic reactive la organización — ver nota del 2026-09-15 en el chat, la cuenta quedó en revisión automática por volumen de pruebas). Mientras tanto, etapa 6 (`cash_payments.py`) sigue pendiente. Migrar el número real de WhatsApp sigue esperando a que el bot esté más completo.
